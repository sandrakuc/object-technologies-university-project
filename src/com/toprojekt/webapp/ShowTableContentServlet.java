...
@WebServlet(name = "ShowTableContent", urlPatterns = {"/ShowTableContent"})
public class ShowTableContentServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<String> content = client.getAllValuesFromTable("0");
        ...
        o.println("<h2 align='center' style='font-weight: bold; font-size: 20px;'>Zawartość tabeli</h2>");
        o.println("</div>");
        ...
        o.println("</thead>");
        o.println("<tbody>");
        for(int i = 0; i < content.size(); i += columns.size()){
            o.println("<tr>");
            for(int j = 0; j < columns.size(); j++){
                o.println("<td>" + content.get(i+j) + "</td>");
            }
            o.println("</tr>");
        }
        o.println("</tbody>");
